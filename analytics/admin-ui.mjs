  ...
  if (document.body.dataset.page === 'login') {
    ...
    const initialize = async () => {
      try {
        ...
        const key = config.siteKey || $('turnstile').dataset.siteKey;
        challengeRequired = Boolean(key);
        if (key) {
          await new Promise((resolve,reject)=>{ const script = document.createElement('script'); script.src = 'https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit'; script.onload=resolve; script.onerror=()=>reject(new Error('安全验证加载失败，请刷新页面。')); document.head.append(script); });
          widget = window.turnstile.render($('turnstile'), {sitekey:key,action:'admin_login',theme:'light',callback:token=>{ challengeToken=token; },'expired-callback':()=>{challengeToken='';},'error-callback':()=>{challengeToken='';status('安全验证失败，请重新验证。',true);}});
        }
