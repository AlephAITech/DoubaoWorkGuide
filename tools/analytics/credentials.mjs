...
const mode=process.argv.includes('--preview')?'preview':'local';
...
await writeFile(loginFile,`豆包统计后台（${mode==='local'?'仅本地开发':'独立云端预览'}）\n账号：admin\n密码：${password}\n\n此文件禁止提交、截图、发到公开聊天或放入部署包。\n${mode==='local'?'地址：http://localhost:8787/admin/analytics\n':'部署后使用该独立 Worker 的 /admin/analytics 地址。\n'}Cloudflare secrets 文件仅含密码哈希，不含此明文密码。\n`,{mode:0o600,flag:'wx'});
